...
class CentralCrop(BaseImageAugmentationLayer):
    ...
    def augment_bounding_boxes(
        ...
        if self.bounding_box_format is None:
            raise ValueError(
                "`CentralCrop()` was called with bounding boxes,"
                "but no `bounding_box_format` was specified in the constructor."
                "Please specify a bounding box format in the constructor. i.e."
                "`CentralCrop(bounding_box_format='xyxy')`"
            )
        ...
        bounding_boxes = tf.cond(
            ...
            lambda: self._resize_bounding_boxes(image, bounding_boxes),
        )
    ...
    def augment_segmentation_mask(
        ...
        return process_segmentation_masks(
            segmentation_mask,
    ...
    def _crop_bounding_boxes(self, image, bounding_boxes, transformation):
        ...
        output["boxes"] = tf.concat(
            [x1 - left, y1 - top, x2 - left, y2 - top], axis=-1
        )
        return output
    ...
    def _resize_bounding_boxes(self, image, bounding_boxes):
        ...
        output["boxes"] = tf.concat(
            [x1 * x_scale, y1 * y_scale, x2 * x_scale, y2 * y_scale], axis=-1
        )
