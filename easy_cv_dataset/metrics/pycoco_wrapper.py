...
def compute_dataset_pycoco_metrics(
    model, dataset, bounding_box_format, sentinel=-1
):
    ...
    ground_truth = {}
    ground_truth["source_id"] = list()
    ground_truth["height"] = list()
    ground_truth["width"] = list()
    ground_truth["num_detections"] = list()
    ground_truth["boxes"] = list()
    ground_truth["classes"] = list()

    predictions = {}
    predictions["source_id"] = list()
    predictions["detection_boxes"] = list()
    predictions["detection_classes"] = list()
    predictions["detection_scores"] = list()
    ...
    for imges, gt_boxes in tqdm(dataset):
        ...
        boxes_pred = bounding_box.convert_format(
            boxes_pred, source=bounding_box_format, target="yxyx"
        )

        for index_0 in range(len(imges)):
            index_source = index_source + 1
            ground_truth["source_id"].append(
                convert_to_tensor(index_source)[None, ...]
            )
            ground_truth["height"].append(
                convert_to_tensor(imges[index_0].shape[-3])[None, ...]
            )
            ground_truth["width"].append(
                convert_to_tensor(imges[index_0].shape[-2])[None, ...]
            )
            gt_valid = gt_boxes["classes"][index_0] != sentinel
            ground_truth["num_detections"].append(
                convert_to_tensor(len(gt_boxes["classes"][index_0][gt_valid]))[
                    None, ...
                ]
            )
            ground_truth["boxes"].append(
                gt_boxes["boxes"][index_0][gt_valid][None, ...]
            )
            ground_truth["classes"].append(
                gt_boxes["classes"][index_0][gt_valid][None, ...]
            )
            ...
            predictions["detection_boxes"].append(
                boxes_pred["boxes"][index_0][:boxes_valid][None, ...]
            )
            predictions["detection_classes"].append(
                boxes_pred["classes"][index_0][:boxes_valid][None, ...]
            )
            predictions["detection_scores"].append(
                boxes_pred["confidence"][index_0][:boxes_valid][None, ...]
            )
            predictions["num_detections"].append(
                boxes_pred["num_detections"][index_0][None, ...]
            )
