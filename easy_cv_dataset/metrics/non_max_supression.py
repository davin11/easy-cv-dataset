#fixed the file: keras_hub/src/layers/modeling/non_max_supression.py
...
class NonMaxSuppression(keras.layers.Layer):
    ...
    def call(
        ...
        class_prediction,
        images=None,
        flag_confidence_threshold = True,
    ):
        ...
        idx, valid_det = non_max_suppression(
            ...
            score_threshold=self.confidence_threshold if flag_confidence_threshold else 0.0,
        )
...
def non_max_suppression(
    ...
    selected_boxes, _, output_size, _ = ops.while_loop(
        ...
        [
            boxes,
            iou_threshold,
            #ops.cast(ops.zeros_like(boxes[:,0,0]), "int32"),
...
def _suppression_loop_body(boxes, iou_threshold, output_size, idx, tile_size):
    ...
    boxes = ops.reshape(boxes, [batch_size, num_tiles*tile_size, 4])
